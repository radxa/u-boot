...
static int __erase_part(char *name)
{
	...
	void *addr = (void *)memalign(CONFIG_SYS_CACHELINE_SIZE,
				      FASTBOOT_ERASE_BUFFER_SIZE);
	if (addr == NULL) {
		pr_err("%s memalign fail\n", __func__);
		return -1;
	}
	...
	if (sunxi_partition_get_info((const char *)name, &info) < 0) {
		...
		printf("********* starting to erase %s partition, this may take some time, please wait for minutes ***********\n",
		       name);
		if (strncmp(name, CONFIG_LAST_PARTITION_NAME,
			    strlen(CONFIG_LAST_PARTITION_NAME)) == 0) {
			if (unerased_sectors >= 16 * 1024 * 1024 / 512)
				unerased_sectors = 16 * 1024 * 1024 / 512;
...
static void __fastboot_erase_part(char *name)
{
	char response[64];
	if (__erase_part(name) < 0) {
		sprintf(response, "erase %s partition fail", name);
	} else {
		sprintf(response, "OKAY");
	}
	__sunxi_fastboot_send_status(response, strlen(response));
...
int sunxi_oem_erase_part(void)
{
	int ret = -1;

	ret = erase_userdata();
	if (ret) {
		pr_err("oem:erase userdata failed!!\n");
		return -1;
	}

	ret = __erase_part("metadata");
	if (ret) {
		pr_err("oem:erase matedata failed!!\n");
		return -1;
	}
	return ret;
}
...
static void __get_var(char *ver_name)
{
	...
	if (!strcmp(ver_name, "version")) {
		...
		sprintf(response + 4, "0x%08x", SUNXI_USB_FASTBOOT_BUFFER_MAX);
		printf("response: %s\n", response);
	} else if (!strncmp(ver_name, "partition-type:", 15)) {
		disk_partition_t info = { 0 };
		if (sunxi_partition_get_info(ver_name + 15, &info) < 0) {
			strcpy(response, "FAIL");
			sprintf(response + 4, "partition %s not found", ver_name + 15);
		} else {
			strcpy(response + 4, (char *)info.type);
		}
		printf("response: %s\n", response);
	} else {
...
static int sunxi_fastboot_write_status_flag(char *name)
{
	...
	if (sunxi_secure_object_write("device_unlock", name, strlen(name))) {
...
static int sunxi_fastboot_state_loop(void *buffer)
{
	...
	case SUNXI_USB_FASTBOOT_SETUP:
		...
		if (strncmp((char *)sunxi_ubuf->rx_req_buffer, "reboot-bootloader",
			...
			__fastboot_erase_part((char *)(sunxi_ubuf->rx_req_buffer + 6));
