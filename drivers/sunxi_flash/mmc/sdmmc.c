...
static int sunxi_flash_mmc_download_boot_param(void)
{
	...
	if (!mmc_boot->block_dev.block_write(&mmc_boot->block_dev,
					       sunxi_flashmap_offset(FLASHMAP_SDMMC, BOOT_PARAM_COMMON),
					       BOOT_PARAM_SIZE / 512, sunxi_boot_param)) {
					 ...
					 unsigned int ext)
{
	...
	if (!sunxi_bootparam_format(sunxi_boot_param)) {
		...
		if (!mmc_sprite->block_dev.block_write(
			    &mmc_sprite->block_dev,
			    sunxi_flashmap_offset(FLASHMAP_SDMMC,
						  BOOT_PARAM_COMMON),
			    sunxi_flashmap_size(FLASHMAP_SDMMC,
						BOOT_PARAM_COMMON),
			    sunxi_boot_param)) {
			pr_err("%s: write boot_param failed\n", __func__);
			return -1;
...
static int sunxi_sprite_mmc_download_boot_param(void)
{
	struct sunxi_boot_param_region *sunxi_boot_param = gd->boot_param;
...
sunxi_flash_desc sunxi_sdmmcs_desc = {
    ...
    .download_boot_param = sunxi_sprite_mmc_download_boot_param,
