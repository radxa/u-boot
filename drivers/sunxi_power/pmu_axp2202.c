...
int pmu_axp2202_reg_debug(void)
{
	...
	if (pmic_bus_read(AXP2202_RUNTIME_ADDR, AXP2202_DATA_BUFFER3, &reg_value[1])) {
		return -1;
	}
	tick_printf("[AXP2202] reboot/charge status: 0x%x = 0x%x\n", AXP2202_DATA_BUFFER3, reg_value[1]);

	if (pmic_bus_read(AXP2202_RUNTIME_ADDR, AXP2202_IIN_LIM, &reg_value[0]))
		return -1;

	pr_msg("[AXP2202] input limit:0x%x\n", reg_value[0]);
