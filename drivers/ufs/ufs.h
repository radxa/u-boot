...
#define UIC_ARG_MIB_SEL(attr, sel)	((((((u32)attr)) & 0xFFFF) << 16) |\
					 ((sel) & 0xFFFF))
#define UIC_ARG_MIB(attr)		UIC_ARG_MIB_SEL(((u32)attr), 0)
						...
						struct ufs_pa_layer_attr *dev_req_params)
{
	if (hba->ops && hba->ops->pwr_change_notify)
	    return hba->ops->pwr_change_notify(hba, status,
...
static inline void ufshcd_ops_device_reset(struct ufs_hba *hba)
{
...
static inline void ufshcd_ops_device_exit(struct ufs_hba *hba)
{
	if (hba->ops && hba->ops->exit) {
		hba->ops->exit(hba);
	}
}
...
#define ufshcd_writel(hba, val, reg)   \
	writel((val), ((u32)((hba)->mmio_base)) + (reg))
#define ufshcd_readl(hba, reg) \
	readl(((u32)((hba)->mmio_base)) + (reg))
