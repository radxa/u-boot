...
static int eqos_start_clks_tegra186(struct udevice *dev)
{
	...
	ret = clk_prepare_enable(eqos->clk_slave_bus);
	if (ret < 0) {
		pr_err("clk_prepare_enable(clk_slave_bus) failed: %d\n", ret);
		goto err;
	}

	ret = clk_prepare_enable(eqos->clk_master_bus);
	if (ret < 0) {
		pr_err("clk_prepare_enable(clk_master_bus) failed: %d\n", ret);
		goto err_disable_clk_slave_bus;
	}

	ret = clk_prepare_enable(eqos->clk_rx);
	if (ret < 0) {
		pr_err("clk_prepare_enable(clk_rx) failed: %d\n", ret);
		goto err_disable_clk_master_bus;
	}

	ret = clk_prepare_enable(eqos->clk_ptp_ref);
	if (ret < 0) {
		pr_err("clk_prepare_enable(clk_ptp_ref) failed: %d\n", ret);
	...
	if (ret < 0) {
		pr_err("clk_prepare_enable(clk_tx) failed: %d\n", ret);
		goto err_disable_clk_ptp_ref;
	}
#endif
...
static int eqos_start_clks_stm32(struct udevice *dev)
{
	...
	if (IS_ERR_OR_NULL(&eqos->clk_ck) && !eqos->clk_ck_enabled) {
		ret = clk_prepare_enable(eqos->clk_ck);
		if (ret < 0) {
			pr_err("clk_prepare_enable(clk_ck) failed: %d\n", ret);
...
static int eqos_start_resets_tegra186(struct udevice *dev)
{
	...
	if (ret < 0) {
		pr_err("gpio_set_value(phy_reset, assert) failed: %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		pr_err("gpio_set_value(phy_reset, deassert) failed: %d\n", ret);
		return ret;
	}
...
static int eqos_get_phy_addr(struct eqos_priv *priv, struct udevice *dev)
{
	...
	return 1;
	if (dev_read_phandle_with_args(dev, "phy-handle", NULL, 1, 0,
				       &phandle_args)) {
...
static int eqos_probe_resources_tegra186(struct udevice *dev)
{
	...
	eqos->phy_reset_gpio = sunxi_name_to_gpio(dev_read_string(dev, "phy-reset-gpios"));
	if (eqos->phy_reset_gpio == -1) {
		pr_err("gpio_request_by_name(phy reset) failed: %d\n", ret);
		goto err_free_reset_eqos;
	}

	eqos->clk_slave_bus = clk_get_by_name(dev, "slave_bus");
	if (IS_ERR(eqos->clk_slave_bus)) {
		ret = PTR_ERR(eqos->clk_slave_bus);
		pr_err("clk_get_by_name(slave_bus) failed: %d\n", ret);
		goto err_free_gpio_phy_reset;
	}

	eqos->clk_master_bus = clk_get_by_name(dev, "master_bus");
	if (IS_ERR(eqos->clk_master_bus)) {
		ret = PTR_ERR(eqos->clk_master_bus);
		pr_err("clk_get_by_name(master_bus) failed: %d\n", ret);
		goto err_free_clk_slave_bus;
	}

	eqos->clk_rx = clk_get_by_name(dev, "rx");
	if (IS_ERR(eqos->clk_rx)) {
		ret = PTR_ERR(eqos->clk_rx);
		pr_err("clk_get_by_name(rx) failed: %d\n", ret);
		goto err_free_clk_master_bus;
	}

	eqos->clk_ptp_ref = clk_get_by_name(dev, "ptp_ref");
	if (IS_ERR(eqos->clk_ptp_ref)) {
		ret = PTR_ERR(eqos->clk_ptp_ref);
		pr_err("clk_get_by_name(ptp_ref) failed: %d\n", ret);
		goto err_free_clk_rx;
		return ret;
	}

	eqos->clk_tx = clk_get_by_name(dev, "tx");
...
static int eqos_probe_resources_stm32(struct udevice *dev)
{
	...
	eqos->clk_master_bus = clk_get_by_name(dev, "stmmaceth");
	if (IS_ERR(eqos->clk_master_bus)) {
		ret = PTR_ERR(eqos->clk_master_bus);
		pr_err("clk_get_by_name(master_bus) failed: %d\n", ret);
		goto err_probe;
	...
	eqos->clk_rx = clk_get_by_name(dev, "mac-clk-rx");
	if (IS_ERR(eqos->clk_rx)) {
		ret = PTR_ERR(eqos->clk_rx);
		pr_err("clk_get_by_name(rx) failed: %d\n", ret);
		goto err_free_clk_master_bus;
	...
	eqos->clk_tx = clk_get_by_name(dev, "mac-clk-tx");
	if (IS_ERR(eqos->clk_tx)) {
		ret = PTR_ERR(eqos->clk_tx);
		pr_err("clk_get_by_name(tx) failed: %d\n", ret);
		goto err_free_clk_rx;
	...
	eqos->clk_ck = clk_get_by_name(dev, "eth-ck");
	if (IS_ERR(eqos->clk_ck)) {
		ret = PTR_ERR(eqos->clk_ck);
...
#if IS_ENABLED(CONFIG_DWC_ETH_QOS_AW)
	{
		.compatible = "allwinner,sunxi-gmac",
		.data = (ulong)&eqos_aw_config
	},
#endif
