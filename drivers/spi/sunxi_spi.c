...
static s32 sunxi_get_spi_mode(unsigned int bus, unsigned int cs)
{
	int nodeoffset = 0;
	int ret = 0;
	u32 rval = 0;
	...
	sprintf(spi_board, "spi%d/spi_board%d", bus, cs);
...
static int sunxi_spi_clk_exit(u32 bus)
{
	struct sunxi_ccm_reg *const ccm =
		(struct sunxi_ccm_reg *)SUNXI_CCM_BASE;
	u8 reset_shift;
				  ...
				  unsigned int max_hz, unsigned int mode)
{
	...
	switch (bus) {
	case 0:
		sunxi_slave->base_addr = SUNXI_SPI0_BASE;
		break;
	case 1:
		sunxi_slave->base_addr = SUNXI_SPI1_BASE;
		break;
	case 2:
		sunxi_slave->base_addr = SUNXI_SPI2_BASE;
		break;
	case 3:
		sunxi_slave->base_addr = SUNXI_SPI3_BASE;
		break;
	default:
		printf("sunxi_spi: invalid base_addr bus %d\n", bus);
	}
#else
	...
	/*rx/tx bus width*/
	ret = sunxi_get_spi_mode(sunxi_slave->bus, cs);
	if (ret > 0) {
...
void spi_free_slave(struct spi_slave *slave)
{
	...
	sunxi_spi_clk_exit(sunxi_slave->bus);
