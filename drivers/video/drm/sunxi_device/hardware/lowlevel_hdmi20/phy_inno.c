...
static int _inno_phy_config_flow(void)
{
	...
	ret = dw_hdmi_wait_event_timeout(_inno_phy_get_rxsense_lock, 10);
	if (ret == 0)
	...
	ret = dw_hdmi_wait_event_timeout(_inno_phy_get_pll_lock, 10);
	if (ret == 0) {
...
int inno_phy_init(void)
{
	...
	struct dw_hdmi_dev_s *hdmi = dw_get_hdmi();
	u32 *tmp_buf;
	ofnode hdmi_node = dev_ofnode(hdmi->dev);
	char phy_name[20];
	...
	/* parse dts */
	read_size = ofnode_read_size(hdmi_node, phy_name);
	if (read_size <= 0) {
	...
	ret = ofnode_read_u32_array(hdmi_node, phy_name, tmp_buf, read_size);
	if (ret < 0) {
