...
************************************************************************************************************
*/
void *sunxi_get_toc0_package(int production_media)
{
	void *ret = NULL;

	if (production_media == STORAGE_NAND) {
		printf("STORAGE_NAND\n");
		ret = Img_OpenItem(imghd, "12345678", "TOC0_NAND0000000");
	} else if (production_media == STORAGE_NOR) {
		printf("STORAGE_NOR\n");
		ret = Img_OpenItem(imghd, "12345678", "TOC0_SPINOR00000");
	} else if (production_media == STORAGE_SD || production_media == STORAGE_EMMC ||
		production_media == STORAGE_EMMC0 || production_media == STORAGE_EMMC3) {
		printf("STORAGE_SD\n");
		ret = Img_OpenItem(imghd, "12345678", "TOC0_SDCARD00000");
	} else if (production_media == STORAGE_UFS) {
		printf("STORAGE_UFS\n");
		ret = Img_OpenItem(imghd, "12345678", "TOC0_UFS00000000");
	}

	//compatible the old scheme
	if (!ret) {
		ret = Img_OpenItem(imghd, "12345678", "TOC0_00000000000");
	}
...
int sunxi_sprite_deal_boot0(int production_media)
{
	...
	if (gd->bootfile_mode == SUNXI_BOOT_FILE_NORMAL ||
		...
		imgitemhd = sunxi_get_toc0_package(production_media);
	}
