				 ...
				 sunxi_certif_info_t *sunxi_certif)
{
	...
	sunxi_key_t *pubkey = &(sunxi_certif->pubkey);
	...
	if (sunxi_sha_calc((u8 *)rotpk_hash, hash_size, (u8 *)align,
			   RSA_BIT_WITDH / 8 * 2)) {
		printf("sunxi_sha_calc: calc  pubkey sha256 with hardware err\n");
		return -1;
	}
	memcpy(out_buf, rotpk_hash, hash_size);
				    ...
				    sunxi_certif_info_t *sub_certif)
{
	...
	sunxi_certif_pubkey_hash_cal(sub_certif, key_hash);
					...
					unsigned cert_len)
{
	u8 hash_of_file[32];
	int ret;
	sunxi_certif_info_t sub_certif = {0};
	void *cert_buf;
	int i = 0;
...
__ERROR_END:
	if (cert_buf)
		free(cert_buf);
	if (sub_certif.aw_cert_info.public_key_info)
		free(sub_certif.aw_cert_info.public_key_info);
	for (i = 0; i < sub_certif.extension.extension_num; i++) {
		if (sub_certif.extension.name[i])
			free(sub_certif.extension.name[i]);
		if (sub_certif.extension.value[i])
			free(sub_certif.extension.value[i]);
	}
	return -1;
}
...
int sunxi_verify_get_rotpk_hash(void *hash_buf)
{
	...
	sunxi_certif_info_t root_certif = {0};
	u8 *buf;
	int ret;
	void *toc1_base;
	...
	ret = sunxi_certif_pubkey_hash_cal(&root_certif, hash_buf);
	if (ret < 0) {
...
int sunxi_verify_toc1_root_cert(void *buf)
{
	...
	ret = sunxi_certif_pubkey_hash_cal(&root_certif, key_hash);
	if (ret < 0) {
...
#define SUNXI_X509_CERTIFF_MAX_LEN 4096
	...
	dm_dev = env_get("verity_dev");

	ret = env_set("dm_dev", dm_dev);
	if (ret) {
		printf("Failed to set dm_dev\n");
		goto out;
	}
			   ...
			   size_t vb_len, const char *pubkey_in_toc1)
{
	...
	salt_buf     = (uint8_t *)memalign(CONFIG_SYS_CACHELINE_SIZE, salt_buf_len);
	if (salt_buf == NULL) {
	...
	flush_cache(ALIGN_DOWN((u32)image_data - salt_buf_len, CONFIG_SYS_CACHELINE_SIZE),
		    ALIGN(salt_buf_len, CONFIG_SYS_CACHELINE_SIZE));
