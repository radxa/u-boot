...
static void boot_jump_linux(bootm_headers_t *images, int flag)
{
	...
	if (!fake) {
		...
		if (sunxi_probe_secure_monitor() && (sunxi_get_force_32bit_os() == 0)) {
			debug("u-boot: Calling sunxi_smc_call_atf(): kernel_entry = 0x%lx, dtb_addr = 0x%lx\n",
				(ulong)kernel_entry, (ulong)r2);
			sunxi_smc_call_atf(ARM_SVC_RUNNSOS, (ulong)kernel_entry, (ulong)r2,  1);
		} else {
			debug("u-boot: Calling kernel_entry()\n");
